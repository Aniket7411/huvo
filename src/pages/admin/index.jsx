...
function Admin() {
  ...
  const getdashboardData = async () => {
    ...
    try {
      ...
      setStoreAddress(response?.sellerDetails?.address[0])
  ...
  console.log("dashboarddashboard", storeAddress)
  return (
        ...
        <div className="p-6 bg-gray-100 rounded-lg shadow-md w-[100vw] md:w-auto">
          ...
          <div className="mt-2 bg-white p-4 rounded-lg shadow-sm">
            ...
            <ul className="list-disc pl-5 text-gray-700 space-y-1">
              <li>{storeAddress?.address}</li>
              <li>{storeAddress?.town}, {storeAddress?.city}</li>
              <li>{storeAddress?.state} - {storeAddress?.postalCode}</li>
              <li>Mobile: {storeAddress?.mobileNumber}</li>
                {/* {!storeAddress?.isDefault && (
